...
def Imputer(raw_data):
        ...
        if imp_col_ch in imp_req_col:
            ...
            Imputer_processing(raw_data,imp_col_ch,imp_teq,imp_req_col)    
        else:
                print("Your entered column name either does not exist or doesn't have any missing value")

def Imputer_processing(raw_data,imp_col_ch,imp_teq,imp_req_col):
    ...
    imp_teq=imp_teq
    imputed_value=imp_value(imp_teq,icpl_ne)
    sep()
    print(f"Program is imputing this value to every missing value in the column '{imp_col_ch}':{imputed_value}")
    sep()
    imp_result_col_py_list=[]
    for element in imp_col_py_list:
        if m.isnan(element):
            imp_result_col_py_list.append(imputed_value)
        else:
    ...
    imp_repeator(imp_req_col,final_imp_result_df)

    
def imp_value(imp_teq,icpl_ne):
    if imp_teq==1:
        ...
        for element in icpl_ne:
            freq[element] = freq.get(element, 0) + 1 #most_frequent = max(freq, key=freq.get)       
        most_frequent=0
        max_frequency=0
        ...
        print("Imputation technique does not exist")

def imp_repeator(imp_req_col,final_imp_result_df):
    while True:
